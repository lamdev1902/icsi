		...
		<div class="container">
			<div class="single-inner list-flex">
				<div class="single-left">
					...
					<?php } 
					$rating_list = get_field('crating_list',$postid);
					if($rating_list) {
					$rating_total = 0;
					foreach($rating_list as $rl) {
						$rating_total += $rl['number'];
					}
					$cprating_number = round($rating_total/count($rating_list), 1);
					?>
					<div class="total-rating">
						<div class="rating-val">
							<div class="the-rating">
								<input type="hidden" class="rating" disabled="disabled" value="<?php echo $cprating_number; ?>" data-empty="fa fa-star" data-filled="fa fa-star" data-fractions="2" />
							</div>
							<span class="number"><?php echo $cprating_number; ?></span>
							<div class="rating-list">
								<a href="" class="show-rating-list">Rating Methodology</a>
								<div class="about-rating">
									<a href="" class="tooltip-close"></a>
									<?php the_field('rating_methodology','option'); ?>
									<ul>
										<?php foreach($rating_list as $rl) { ?>
										<li>
											<div class="name"><?php echo $rl['name']; ?></div>
											<div class="rating-start">
											<input type="hidden" class="rating" disabled="disabled" value="<?php echo $rl['number']; ?>" data-empty="fa fa-star-o" data-filled="fa fa-star" data-fractions="2" />
											</div>
										</li>
										<?php } ?>
									</ul>
...
<script>
jQuery(function($) {
	$('.show-rating-list').on('click',function() {
		$('.about-rating').toggleClass('active');
		return false;
	});
	...
	$(document).on('click', function (e) {
	    if ($(e.target).closest(".about-rating").length === 0) {
	        $(".about-rating").removeClass('active');
	    }
	});
});
</script>